...
Remember the example we discussed in the class: how does a student learn to solve equations? Then looking at the interface of a math tutor (see Figure \@ref(mathtutor)), try to explain the problem-solving practice of a student and how the ITS captures the declarative and procedural knowledge.

```{r mathtutor, fig.cap="An example interface of a math tutor, taken from the [CMU Math Tutors](https://mathtutor.web.cmu.edu/home)", echo=FALSE, message=FALSE, warning=FALSE}
...
The fundamental architecture of an ITS consists of four components (see Figure \@ref(itsarchitecture)):
...
```{r itsarchitecture, fig.cap="The architecture of an Intelligent Tutoring System", echo=FALSE, message=FALSE, warning=FALSE}
